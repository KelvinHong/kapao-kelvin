...
def create_mini_annotations(split: Literal["train", "val"], sample_size: int = 16, seed: int = 0):
    ...
    new_annotation_path = os.path.join(ANNOTATION_DIR, f"person_keypoints_{split}2017_mini.json")
    ...
    selected_image_paths = [os.path.join(DATASET_DIR, "images", f"{split}2017", img["file_name"]) for img in selected_images]
    
    selected_annotations = [ann for ann in annotations if ann["image_id"] in selected_ids]
    logger.info(f"Selected {len(selected_images)} images and {len(selected_annotations)} annotations.")
